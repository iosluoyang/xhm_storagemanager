<template>
	<view class="stockview">
		...
		<view class="proinfoview margin padding radius shadow-warp bg-white" v-if="product">
			...
			<view class="baseinfoview flex flex-direction">
				...
				<view class="flex align-center margin-bottom-sm">
					<!-- 商品规格信息 -->
					<view class="selectspecview cu-tag round bg-gradual-green margin-right" :class=" ifshowbtnanimationspec ? 'animation-shake' : '' " @tap.stop="ifshowpopup=true">
						{{ showspecstr ? showspecstr : i18n.tip.pleaseselectgoodspec }}
					</view>
					
					<!-- 商品库存信息 -->
					<text v-if="selectstockinfo" class="text-grey stockview">{{ `${i18n.goods.stock}: ` }} <text class="text-bold text-black">{{selectstockinfo.stockCount}}</text> </text>
				</view>
		...
		<view class="cu-bar btn-group">
			<button class="cu-btn round lg shadow-blur line-green" :class=" ifshowbtnanimationin ? 'animation-shake' : '' " @tap.stop="fixstock('in')">{{i18n.stock.stockin}}</button>
			<button class="cu-btn round lg shadow-blur bg-green" :class=" ifshowbtnanimationout ? 'animation-shake' : '' " @tap.stop="fixstock('out')">{{i18n.stock.stockout}}</button>
		</view>
	...
	export default {
		...
		data() {
			return {
				...
				selectstockinfo: null, // 当前选中的库存规格信息
		...
		methods: {
			...
			fixstock(stocktype) {
				...
				// 出库
					...
					// 校验出库数量
					if(parseInt(stockNum) > _this.selectstockinfo.stockCount || stockNum === '' || parseInt(stockNum) === 0) {
				...
				uni.showModal({
					...
					success: res => {
						if(res.confirm) {
							...
							let data = {
								pid: _this.pid,
								specId: _this.selectstockinfo.specId,

  ...
  "email_confirmed_localdata": [
    ...
    },
    {
      ...
      "value": 1
        ...
        rules: {
          ...getValidator(["nickname","gender","status","mobile","mobile_confirmed","email","email_confirmed","avatar","comment","realname_auth","my_invite_code","username"])
        }
      }
    },
    onLoad(e) {
    ...
    methods: {
      ...
      getDetail(id) {
        ...
        db.collection(dbCollectionName).doc(id).field('username,password_secret_version,nickname,gender,status,mobile,mobile_confirmed,email,email_confirmed,avatar,wx_unionid,wx_openid,ali_openid,apple_openid,comment,realname_auth,last_login_date,last_login_ip,token,inviter_uid,invite_time,my_invite_code').get().then((res) => {

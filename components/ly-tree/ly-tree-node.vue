	...
	<view class="ly-tree-node" @tap.stop="handleClick" v-show="node.visible" :class="{ 'is-expanded': expanded, 
	 ...
	 role="treeitem" name="LyTreeNode" ref="node">
		<view class="ly-tree-node__content" :style="{ 'padding-left': (node.level - 1) * indent + 'px' }">
			...
			<view v-show="treeStatus==='edit'" class="optionview flex align-center margin-left">
				
				<!-- 新增按钮  一级分类和非系统级别的节点才显示 -->
				<text v-if="node.level < 2 && node.data.sysFlag !== 1" class="cuIcon cuIcon-add text-bold shadow margin-right" @tap.stop="handleAddNode"></text>
				
				<!-- 编辑按钮 非系统级别节点才显示 -->
				<text v-if="node.data.sysFlag !== 1" class="cuIcon cuIcon-edit margin-right" @tap.stop="handleEditNode"></text>
				
				<!-- 删除按钮 非系统级别节点才显示 -->
				<text v-if="node.data.sysFlag !== 1" class="cuIcon text-red cuIcon-delete" @tap.stop="handleDeleteNode"></text>
...
<script>
	...
	export default {
		...
		props: {
			// 当前tree状态  normal 为正常状态  edit为编辑状态
			treeStatus: {
				type: String,
				default: 'normal',
			},
		...
		methods: {
			...
			handleAddNode() {
				if(this.node.disabled) return;
				this.tree.$emit('add-node', this.node)
			},
			
			// 点击编辑节点
			handleEditNode() {
				if(this.node.disabled) return;
				this.tree.$emit('edit-node', this.node)
			},
			
			// 点击删除节点
			handleDeleteNode() {
				if(this.node.disabled) return;
				this.tree.$emit('delete-node', this.node)
			},
